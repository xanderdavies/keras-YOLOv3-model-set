...
def get_yolo3_model(model_type, num_feature_layers, num_anchors, num_classes, input_tensor=None, input_shape=None, model_pruning=False, pruning_end_step=10000):
    ...
    if num_feature_layers == 2:
        if model_type in yolo3_tiny_model_map:
            ...
            raise ValueError(f'{model_type} model type is not supported now in yolo3_tiny_model_map')
    ...
    #YOLOv3 model has 9 anchors and 3 feature layers
        ...
        if model_type in yolo3_model_map:
            ...
            raise ValueError(f'{model_type} model type is not supported now in yolo3_model_map')
    else:
...
def get_yolo3_train_model(model_type, anchors, num_classes, from_coco=False, weights_path=None, freeze_level=1, optimizer=Adam(lr=1e-3, decay=0), label_smoothing=0, elim_grid_sense=False, model_pruning=False, pruning_end_step=10000):
